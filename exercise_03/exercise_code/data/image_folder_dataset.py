...
class ImageFolderDataset(Dataset):
    ...
    def make_dataset(directory, class_to_idx):
        ...
        ########################################################################
        for cls,index in (sorted(class_to_idx.items())):
            DIR = os.path.join(directory,cls)
            for i in range(len(os.listdir(DIR))):
                           image_path=os.path.join(DIR,str(i+1).zfill(4) + ".png")
    ...
    def __getitem__(self, index):
        ...
        pass
        #x=self.load_image_as_numpy(self.images[index])
        y=self.labels[index]
        if self.transform == None:
            x=np.asarray(Image.open(self.images[index]))
        else:
            x=self.transform(np.asarray(Image.open(self.images[index])))
